...
function et_show_ajax_project(){
	...
	while ( $portfolio_query->have_posts() ) : $portfolio_query->the_post();
		...
		echo '<a class="more" href="' . get_permalink() . '">' . __( 'Leia Mais &raquo;', 'Flexible' ) . '</a>';
...
/* Funcao para adicionar o slug da categoria no wp_list_categories */
class donp_wp_list_categories extends Walker_Category {
    ...
    public function start_el(&$output, $category, $depth, $args) {
        parent::start_el( $output, $category, $depth, $args );
                ...
                $replace = 'cat-item-' . $category->term_id . ' cat-'. $category->slug .'"';
...
/* Funcao para criar pagina sempre que um term na taxonomia agenda-categoria for criado */
function donp_create_page_description( $term_id, $tt_id, $taxonomy ) {
	$term = get_term_by('id', $tt_id, 'agenda-categoria');
	$post = array();
	$post['post_type']    = 'page';
	$post['post_content'] = 'Adicione aqui a descrição para a categoria <strong>'.$term->name.'</strong>.';
