...
class fslider_html
{
	...
	*/
	...
	function html_1($slider, $button, $id, $images)
	{
		...
		{
			$image = get_stylesheet_directory_uri() . '/includes/flexible-slider/images/' . $id . '/crop/' . $image;
	...
	function script_3($slider, $button, $width, $height, $drag, $auto, $timer, $infinite, $keyboard, $images, $id)
	{
		...
		<style>
			...
			<?php for( $i = 0; $i < sizeof($images); $i++ ) : $image = get_stylesheet_directory_uri() . '/includes/flexible-slider/images/' . $id . '/button/' . $images[$i]; ?>
	...
	function html_4($slider, $button, $id, $images, $slidetitles, $links, $targets, $descriptions)
	{
		...
		//$slidetitles = array_reverse($slidetitles);
		//$links = array_reverse($links);
		//$targets = array_reverse($targets);
		//$descriptions = array_reverse($descriptions);
		$html = "<div id='$slider'><div class = 'slider slider-style-4'>";

		for($i = 0; $i < sizeof($images); $i++)
		{
			$image = get_stylesheet_directory_uri() . '/includes/flexible-slider/images/' . $id . '/crop/' . $images[$i];
			$html .= "<div class='item'><div class='img'><img src='$image' /></div><div class='caption caption-4'>";
			...
			if( $slidetitles[$i] ) $html .= "<div class = 'bg bg-4'></div>";
			
			$html .= "</div></div>";
			
			$count_full++;
			if( $count_full == 4 ) break;
		}
		...
		for($i = 0; $i < sizeof($images); $i++)
		{
			// if( $links[$i] ) $title = "<p class='title-desc'><a href='$links[$i]' $target>".base64_decode($slidetitles[$i])."</a></p>";
			// else $title = "<p class='title-desc'>".base64_decode($slidetitles[$i])."</p>";

			$html .= "<div class = 'button' id = '$button-item$i'>";
						...
						$titlefont, $titlecolor, $titlesize, $descfont, $desccolor, $descsize)
	{
		...
		<style>
			...
			#<?php echo $slider ?> .prevButton {
				...
				width: 20px;
				height: 40px;
				background: url(<?php echo get_stylesheet_directory_uri(); ?>/includes/flexible-slider/assets/images/slider-buttons.png) no-repeat 0 0;
				margin-top: -20px;
				z-index: 2;
			}
			...
			#<?php echo $slider ?> .nextButton {
				...
				width: 20px;
				height: 40px;
				background: url(<?php echo get_stylesheet_directory_uri(); ?>/includes/flexible-slider/assets/images/slider-buttons.png) no-repeat 100% 0;
				margin-top: -20px;
				z-index: 2;
			}
		</style>
